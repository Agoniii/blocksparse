    ...
    with ops.name_scope("gradients"):
        ...
        for i, dy in enumerate(grad_ys):
            if dy is None:
                ...
                grad_ys[i] = tf.fill(tf.shape(ys[i]), tf.constant(1.0, dtype=ys[i].dtype, name="grad_ys_%d"%i))
            ...
            if any(dy is not None for dy in dys) and op not in stop_ops:
                ...
                with ops.name_scope(op.name + "_grad"):
                    ...
                    if len(dxs) != len(op.inputs):
                        raise ValueError("Num gradients %d generated for op %s do not match num inputs %s" % (len(dxs), op.node_def, len(op.inputs)))
            ...
            for i, (x, dx) in enumerate(zip(op.inputs, dxs)):
                if dx is not None:
                    ...
                    try:
                        ...
                        raise ValueError("Incompatible shapes between op input %s and calculated input gradient %s for %s (idx:%d)" % (x.shape, dx.shape, op.name, i))
